...
const ms = require("ms");
const { VoiceState } = require("../Database/index");
...
module.exports = new PlayerEvent(
   ...
   */ async (client, queue, track) => {
    if (client.collectors.get(queue.guild.id)) return;
    ...
    embed
      ...
      .setDescription(
        ` ${CatJam.emoji} | [**\`${track.title}\`**](${track.url}) - <@!${track.requestedBy.id}>`
      )
      .setAuthor({
    ...
    const filter = (i) => {
      if (
        i.user.id === track.requestedBy.id ||
    ...
    collector.on(
       ...
       */ async (i) => {
        ...
        if (i.customId === "pauseResume") {
          ...
          return message?.edit({ components: [row1, row2] }).catch((err) => {});
          ...
          const timestamp = await VoiceState.findOne({
